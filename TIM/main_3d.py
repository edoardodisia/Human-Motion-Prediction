...
def main(opt):
    ...
    script_name = script_name + '_3D_in{:d}_out{:d}'.format(opt.input_n, opt.output_n)
    ...
    # conversione tensore in base a HW disponibile (cpu o gpu)
    model.to(MY_DEVICE)
    ...
    train_dataset = H36motion3D(path_to_data=opt.data_dir, actions='all', input_n=input_n, output_n=output_n,
                                split=0, sample_rate=sample_rate)

    acts = data_utils.define_actions('all')
    test_data = dict()
    test_data2 = dict()

    # N.B: i due dizionari vengono popolati aggiungiendo oggetti "H36motion3D" con la relativa chiave
    # dell'azione a cui sono associati!
    for act in acts:
        ...
        test_dataset2 = H36motion3D(path_to_data=opt.data_dir, actions=act, input_n=input_n, output_n=output_n, split=1,
                                    sample_rate=sample_rate, treat_subj5_differently=False)
    ...
    val_dataset = H36motion3D(path_to_data=opt.data_dir, actions='all', input_n=input_n, output_n=output_n,
                              split=2, sample_rate=sample_rate)
    ...
    # inizio effettivo del training in base al numero di epoche specificate
    for epoch in range(start_epoch, opt.epochs):
        ...
        # per epoch
        lr_now, t_l = train(train_loader, model, optimizer, lr_now=lr_now, max_norm=opt.max_norm,
                            dim_used=train_dataset.dim_used)
        ret_log = np.append(ret_log, [lr_now, t_l])
        head = np.append(head, ['lr', 't_l'])

        v_3d = val(val_loader, model, dim_used=train_dataset.dim_used)
        ...
        for act in acts:
            ...
            test_l, test_3d = test(test_data[act], model, input_n=input_n, output_n=output_n,
                                   dim_used=train_dataset.dim_used)
            ...
            # This is something we used as a way to reduce the variance of the test loss
            # We don't use this in the paper such that we can compare our results with other works
            test_l, test_3d = test(test_data2[act], model, input_n=input_n, output_n=output_n,
                                   dim_used=train_dataset.dim_used)
            ret_log = np.append(ret_log, test_3d)
...
def train(train_loader, model, optimizer, lr_now=None, max_norm=True, dim_used=[]):
    t_l = utils.AccumLoss()

    model.train()
    st = time.time()
    bar = Bar('>>>', fill='>', max=len(train_loader))
    for i, (inputs1, all_seq1) in enumerate(train_loader):
...
def test(train_loader, model, input_n=20, output_n=50, dim_used=[]):
    N = 0
    t_l = 0
    if output_n == 25:
...
def val(train_loader, model, dim_used=[]):
