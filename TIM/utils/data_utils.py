...
import os
from NN1.utils import forward_kinematics
...
def readCSVasFloat(filename):
    ...
    for line in lines:
        line = line.strip().split(',') # rimozione spazi bianchi -> suddivisione tramite ","
        if len(line) > 0:
            # conversione char -> float32 per ogni carattere nella stringa associata
            # alla riga della matrice
            floatArray.append(np.array([np.float32(x) for x in line]))
...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    for action_idx in np.arange(nactions):
        ...
        for examp_index in np.arange(count):
            ...
            action_sequence = readCSVasFloat(filename)
            n, d = action_sequence.shape
            exptmps = Variable(torch.from_numpy(action_sequence)).float().to(MY_DEVICE)
...
def rotmat2euler_torch(R):
    ...
    if len(idx_spec2) > 0:
        R_spec2 = R[idx_spec2, :, :]
        eul_spec2 = Variable(torch.zeros(len(idx_spec2), 3).float()).to(MY_DEVICE)
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = Variable(torch.zeros(len(idx_remain), 3).float()).to(MY_DEVICE)
...
def rotmat2quat_torch(R):
    ...
    q = Variable(torch.zeros(R.shape[0], 4)).float().to(MY_DEVICE)
...
def expmap2rotmat_torch(r):
    ...
    n = r1.data.shape[0]
    R = Variable(torch.eye(3, 3).repeat(n, 1, 1)).float().to(MY_DEVICE) + torch.mul(
...
def expmap2xyz_torch(expmap):
    ...
    :return: N*32*3
    """
    # N.B: "N" corrisponde agli istanti temporali di cui si conosce la posizione dei joint, ovvero i frame catturati!
    
    # creazione variabili parametrizzazione nello spazio. "parent" corrisponde al kinematic node
    parent, offset, rotInd, expmapInd = forward_kinematics._some_variables()

    # conversione parametrizzazione -> coordinate x,y,z (cinematica diretta)
        ...
        for action_idx in np.arange(len(actions)):
            ...
            if not (subj == 5) or not treat_subj5_differently:
                for subact in [1, 2]:  # subactions
                    ...
                    filename = '{0}/S{1}/{2}_{3}.txt'.format(path_to_dataset, subj, action, subact)
                    matrixActionSeq = readCSVasFloat(filename) # ottengo matrice (n righe = righe file .txt letto!!!)

                    # ottengono dimensioni matrice. "d" = 99 è fisso (legato ai joints?) mentre "n" è variabile (frame catturati diversi?)
                    # Di fatto ogni riga della matrice corrisponde ad un istante temporale di cui vengono fornite le 
                    # posizioni dei joint del soggetto
                    n, d = matrixActionSeq.shape
                    print("Matrix (float32) {0}x{1} (nxd)".format(n, d))

                    # N.B: il valore di default di sample rate è 2!
                    rowsFiltered = range(0, n, sample_rate)

                    totFrames = len(rowsFiltered) # equivalente a scrivere n/sample_rate
                    print("Frames captured -> " + str(totFrames))

                    # selezione sottoinsieme righe filtrate "ogni sample rate"
                    arrayFiltered = np.array(matrixActionSeq[rowsFiltered, :])
                    print("Array sequence shape -> " + str(arrayFiltered.shape))

                    # "torch.from_numpy" crea un tensore a partire da un array. Successivamente
                    # viene specificato il tipo di dato associato all'array di cui si farà carico
                    # il tensore, che in questo caso è "float32" (SOLO INIZIALIZZATO!!!)
                    tensorExpMap = Variable(torch.from_numpy(arrayFiltered)).float().to(MY_DEVICE)
                    print("Tensor shape: " + str(tensorExpMap.shape))

                    # remove global rotation and translation (inizializzazione prime 6 colonne della matrice)
                    tensorExpMap[:, 0:6] = 0
                    print("Sequence before 3D: " + str(tensorExpMap))

                    pointsXYZ = expmap2xyz_torch(tensorExpMap)

                    print("Pose 3D (32 joints if HM36): " + str(pointsXYZ))

                    arrayFiltered = pointsXYZ.view(totFrames, -1).cpu().data.numpy()

                    fs = np.arange(0, totFrames - seq_len + 1)
                    fs_sel = fs

                    for i in np.arange(seq_len - 1):
                        fs_sel = np.vstack((fs_sel, fs + i + 1))
                    fs_sel = fs_sel.transpose()
                    seq_sel = arrayFiltered[fs_sel, :]

                    if len(sampled_seq) == 0:
                        sampled_seq = seq_sel
                        complete_seq = arrayFiltered
                    else:
                        sampled_seq = np.concatenate((sampled_seq, seq_sel), axis=0)
                        complete_seq = np.append(complete_seq, arrayFiltered, axis=0)
            else:
                print("Reading subject {0}, action {1}, subaction {2}".format(subj, action, 1))
                filename = '{0}/S{1}/{2}_{3}.txt'.format(path_to_dataset, subj, action, 1)
                matrixActionSeq = readCSVasFloat(filename)
                n, d = matrixActionSeq.shape
                rowsFiltered = range(0, n, sample_rate)

                num_frames1 = len(rowsFiltered)
                the_sequence1 = np.array(matrixActionSeq[rowsFiltered, :])
                the_seq1 = Variable(torch.from_numpy(the_sequence1)).float().to(MY_DEVICE)
                the_seq1[:, 0:6] = 0
                ...
                num_frames2 = len(rowsFiltered)
                the_sequence2 = np.array(matrixActionSeq[rowsFiltered, :])
                the_seq2 = Variable(torch.from_numpy(the_sequence2)).float().to(MY_DEVICE)
                the_seq2[:, 0:6] = 0
                ...
                if len(sampled_seq) == 0:
                    ...
                    # iterazioni successive => concatenazione
...
if __name__ == "__main__":
    ...
    r = Variable(torch.from_numpy(r)).float().to(MY_DEVICE)
