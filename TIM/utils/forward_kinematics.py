...
def _some_variables():
    ...
    # padre = i-esimo elemento, figlio = i+1-esimo elemento
    ...
    offset = offset.reshape(-1, 3) # cambio dimensioni array (stessi dati). "-1" -> calcolo automatico della dimensione compatibile
...
def fkl_torch(angles, parent, offset):
    ...
    :return: N*joint_n*3
    """
    n = angles.data.shape[0] # seleziono m righe matrice angles
    j_n = offset.shape[0] # seleziono n righe matrice offset (calcolate automaticamente in "_some_variables")
    ...
    # poichè devo ruotare una matrice di punti nxj_n ogni elemento di questa struttura
    # sara' una matrice di rotazione Rij = 3x3, essendo nello spazio con ogni punto P = (xp,yp,zp)
